...
BASEDIR=$(dirname "$0")
PROTO_DIR="proto"
TARGET_DIR="../python/ofx/api/proto"
...
generate_all() {
  local recursive_flag=$1
  for sub_dir in "$BASEDIR"/*; do
    if [ -d "$sub_dir" ] && [ "$sub_dir" != "$BASEDIR/$PROTO_DIR" ]; then
      for proto in "$sub_dir"/*.proto; do
        generate "$sub_dir" "$TARGET_DIR/${sub_dir##*/}" "$proto"
      done
      # Handle recursive flag
      [ "$recursive_flag" == "true" ] && find "$sub_dir" -type d -exec bash -c 'generate_all "$1" "$2"' _ {} \;
    fi
  done
}
