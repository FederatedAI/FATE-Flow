...
| eggroll           | rolllsite      | 9370      | Cross-site or cross-party communication fate_components   |
...
In this example, there is only one host on each end, and multiple hosts on each end. Currently, only nodemanager multi-node deployment is supported, and other fate_components are single-node.
...
```
#Because fate_flow depends on more fate_components, there may be exceptions in startup. The processing is as follows:
netstat -tlnp | grep 9360
...
```
#Note: Because mysql is a basic component, the startup is slow. It is recommended to restart all fate_components first, then start mysql first, and then start other fate_components
sh service.sh start/stop/restart/status all

#Note: Because fateflow depends on many fate_components, restarting all operations may cause fateflow to start abnormally. The processing is as follows:
netstat -tlnp | grep 9360
If there is no port, restart fateflow:
sh service.sh stop fate-fateflow
sh service.sh start fate-fateflow
