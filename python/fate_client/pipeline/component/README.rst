...
There are two classes of ``input``: ``data`` and ``model``. Not all fate_components have
both classes of input, and a component may accept only some types of the class.
For information on each fate_components' input, check the `list <#component-list>`_ below.
...
in a FATE job. Not all fate_components have both classes of outputs.
For information on each fate_components' output, check the `list <#component-list>`_ below.
...
Meanwhile, to download fate_components' output table or model, please use `task info <#task-info>`_ interface.
...
All input and output data of fate_components need to be wrapped into ``Data``
objects when being passed between fate_components. For information on valid data
types of each component, check the `list <#component-list>`_ below.
Here is a an example of chaining fate_components with different types of data input and output:
...
.. warning::
   ...
   Please note that when using `stepwise` or `cross validation` method, fate_components do
   not have ``model`` output. For information on valid model
   types of each fate_components, check the `list <#component-list>`_ below.
...
Pipeline task info API. Currently Pipeline support these four types of query on fate_components:
