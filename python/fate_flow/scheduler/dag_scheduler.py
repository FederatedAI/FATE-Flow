...
from fate_flow.db.db_models import DB, Job, Task, JobSchedulerInfo
...
from fate_flow.settings import ENABLE_MODEL_STORE, PARTY_ID
...
class DAGScheduler(Cron):
    ...
    def submit(cls, submit_job_conf: JobConfigurationBase):
        job_id = job_utils.generate_job_id() if not submit_job_conf.job_id else submit_job_conf.job_id
        submit_result = {
            "job_id": job_id
        }
        ...
        try:
            ...
            job = JobSchedulerInfo()
            job.f_job_id = job_id
            job.f_dsl = dsl
            job.f_roles = runtime_conf["role"]
            job.f_initiator_party_id = runtime_conf.get("initiator_party_id", "")
            job.f_scheduler_party_id = runtime_conf.get("scheduler_party_id") if runtime_conf.get("scheduler_party_id")\
                else PARTY_ID
            JobController.create_common_job_parameters(job_id=job.f_job_id, common_job_parameters=common_job_parameters)
            job.f_runtime_conf = conf_adapter.update_common_parameters(common_parameters=common_job_parameters)
            auto_retries = common_job_parameters.auto_retries if common_job_parameters.auto_retries else 0
            job.f_status = StatusSet.READY
            JobSaver.create_job(job.to_human_model_dict(), is_scheduler=True)
            status_code, response = FederatedScheduler.create_job(job_id, job.f_roles, job.to_human_model_dict())
                ...
                for role in response:
                    ...
                    for party, res in response[role].items():
                        ...
                        if not all_components:
                            all_components = [name for name in response[role][party]["data"]["components"].keys()]
                for role, party_ids in job.f_roles.items():
                    for party_id in party_ids:
                        cls.initialize_scheduler_tasks(job_id, role, party_id, need_run_components[role][party_id], auto_retries)
                job.f_status = JobStatus.WAITING
                cls.initialize_scheduler_tasks_status(job_id, all_components)
                status_code, response = FederatedScheduler.sync_job_status(job_id=job.f_job_id, roles=job.f_roles,
                                                                           status=job.f_status,
                                                                           command_body=job.to_human_model_dict())
                if status_code != FederatedSchedulingStatusCode.SUCCESS:
                    raise Exception(f"set job to waiting status failed: {response}")
                JobSaver.update_job_status({"job_id": job.f_job_id, "status": job.f_status}, is_scheduler=True)
    ...
    @classmethod
    def initialize_scheduler_tasks(cls, job_id, role, party_id, component_list, auto_retries):
        for component_name in component_list:
            ...
            JobSaver.create_task(task_info, is_scheduler=True)

    @classmethod
    def initialize_scheduler_tasks_status(cls, job_id, component_list):
        for component_name in component_list:
            task_info = {
                "job_id": job_id,
                "component_name": component_name,
            ...
            JobSaver.create_task_scheduler_status(task_info)
    ...
    def update_parameters(cls, job, job_parameters, component_parameters):
        ...
        status_code, response = FederatedScheduler.update_parameter(job_id=job.f_job_id,
                                                                    roles=job.f_roles,
    ...
    def run_do(self):
        ...
        jobs = JobSaver.query_job(status=JobStatus.WAITING, order_by="create_time", reverse=False, is_scheduler=True)
        ...
        jobs = JobSaver.query_job(status=JobStatus.RUNNING, order_by="create_time", reverse=False, is_scheduler=True)
        ...
        jobs = JobSaver.query_job(ready_signal=True, order_by="create_time", reverse=False, is_scheduler=True)
        ...
        jobs = JobSaver.query_job(status=set(EndStatus.status_list()), end_time=[current_timestamp() - JobDefaultConfig.end_status_job_scheduling_time_limit, current_timestamp()],
                                  is_scheduler=True)
    ...
    def schedule_waiting_jobs(cls, job: JobSchedulerInfo):
        job_id = job.f_job_id
        if job.f_cancel_signal:
            job.f_status = JobStatus.CANCELED
            FederatedScheduler.sync_job_status(job_id=job.f_job_id, roles=job.f_roles, status=job.f_status,
                                               command_body=job.to_human_model_dict())
            JobSaver.update_status(JobSchedulerInfo, {"job_id": job.f_job_id, "status": job.f_statusG})
        ...
        dependence_status_code, federated_dependence_response = FederatedScheduler.dependence_for_job(job_id=job_id,
                                                                                                      roles=job.f_roles)
        ...
        if dependence_status_code == FederatedSchedulingStatusCode.SUCCESS:
            apply_status_code, federated_response = FederatedScheduler.resource_for_job(job_id=job.f_job_id,
                                                                                        roles=job.f_roles,
                                                                                        operation_type=ResourceOperation.APPLY)
            if apply_status_code == FederatedSchedulingStatusCode.SUCCESS:
                cls.start_job(job_id=job_id, roles=job.f_roles)
            else:
                # rollback resource
                rollback_party = {}
                failed_party = {}
                ...
                if rollback_party:
                    return_status_code, federated_response = FederatedScheduler.resource_for_job(job_id=job_id,
            ...
            if not retcode_set.issubset({RetCode.RUNNING, RetCode.SUCCESS}):
                FederatedScheduler.stop_job(job.f_job_id, roles=job.f_roles, stop_status=StatusSet.FAILED)

    @classmethod
    def check_component(cls, job, check_type="inheritance"):
        schedule_logger(job.f_job_id).info("component check")
        dependence_status_code, response = FederatedScheduler.check_component(job_id=job.f_job_id, roles=job.f_roles,
        ...
        if component_set != set(job.f_inheritance_info.get("component_list")):
            ...
            status_code, response = FederatedScheduler.align_args(job.f_job_id, job.f_roles, command_body=command_body)
    ...
    def schedule_rerun_job(cls, job):
        if EndStatus.contains(job.f_status):
            ...
            status, response = FederatedScheduler.sync_job_status(job_id=job.f_job_id, roles=job.f_roles, status=job.f_status,
                                                   command_body=job.to_human_model_dict())
            if status == FederatedSchedulingStatusCode.SUCCESS:
                schedule_utils.rerun_signal(job_id=job.f_job_id, set_or_reset=False)

                FederatedScheduler.sync_job(job_id=job.f_job_id, roles=job.f_roles,
                                            command_body=job.to_human_model_dict(
                                                only_primary_with=["ready_signal", "ready_time", "rerun_signal",
                                                                   "progress", "end_time", "elapsed"])
                                            )
    ...
    def start_job(cls, job_id, roles):
        schedule_logger(job_id).info(f"start job on scheduler")
        FederatedScheduler.start_job(job_id, roles)
        JobSaver.update_job_status(job_info={"job_id": job_id, "status": StatusSet.RUNNING}, is_scheduler=True)

    @classmethod
    # @schedule_utils.schedule_lock
    def schedule_running_job(cls, job: JobSchedulerInfo, force_sync_status=False):
        ...
        if new_job_status != job.f_status or new_progress != job.f_progress:
            ...
            if int(new_progress) - job.f_progress > 0:
                ...
                FederatedScheduler.sync_job(job_id=job.f_job_id,
                                            roles=job.f_roles,
                                            command_body=job.to_human_model_dict(only_primary_with=["progress"]))
                cls.update_job_on_scheduler(scheduler_job=job, update_fields=["progress"])
            if new_job_status != job.f_status:
                job.f_status = new_job_status
                if EndStatus.contains(job.f_status):
                    FederatedScheduler.save_pipelined_model(job_id=job.f_job_id, roles=job.f_roles)
                FederatedScheduler.sync_job_status(job_id=job.f_job_id, roles=job.f_roles, status=job.f_status,
                                                   command_body=job.to_human_model_dict())
                cls.update_job_on_scheduler(scheduler_job=job, update_fields=["status"])
        if EndStatus.contains(job.f_status):
    ...
    def set_job_rerun(cls, job_id, initiator_role, initiator_party_id, auto, force=False,
        ...
        schedule_logger(job_id).info(f"try to rerun job on scheduler {initiator_role} {initiator_party_id}")

        jobs = JobSaver.query_job(job_id=job_id, is_scheduler=True)
        if not jobs:
            raise RuntimeError(f"can not found job {job_id} on scheduler")
        job = jobs[0]
    ...
    def get_rerun_component(cls, component_name, job, dsl_parser, force):
        if not component_name or component_name == job_utils.PIPELINE_COMPONENT_NAME:
            ...
            dependence_status_code, response = FederatedScheduler.check_component(job.f_job_id, job.f_roles,
                                                                                  check_type="rerun")
            success_task_list = [task.f_component_name for task in JobSaver.query_task(job_id=job.f_job_id,
    ...
    @classmethod
    def update_job_on_scheduler(cls, scheduler_job: JobSchedulerInfo, update_fields: list):
        schedule_logger(scheduler_job.f_job_id).info(f"try to update job {update_fields} on scheduler")
        jobs = JobSaver.query_job(job_id=scheduler_job.f_job_id, is_scheduler=True)
        if not jobs:
            raise Exception("Failed to update job status on scheduler")
        job_info = scheduler_job.to_human_model_dict(only_primary_with=update_fields)
        for field in update_fields:
            job_info[field] = getattr(scheduler_job, "f_%s" % field)
        JobSaver.update_job_status(job_info=job_info, is_scheduler=True)
        JobSaver.update_job(job_info=job_info, is_scheduler=True)
        schedule_logger(scheduler_job.f_job_id).info(f"update job {update_fields} on scheduler finished")
    ...
    def stop_job(cls, job_id, stop_status):
        ...
        if len(jobs) > 0:
            ...
            status_code, response = FederatedScheduler.stop_job(job_id=job_id, roles=job.f_roles, stop_status=stop_status)
            if status_code == FederatedSchedulingStatusCode.SUCCESS:
    ...
    def finish(cls, job, end_status):
        ...
        FederatedScheduler.clean_job(job_id=job.f_job_id, roles=job.f_roles, command_body=job.f_runtime_conf["role"].copy())
