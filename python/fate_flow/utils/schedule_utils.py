...
from fate_flow.db.db_models import DB, Job, JobSchedulerInfo
...
def ready_signal(job_id, set_or_reset: bool, ready_timeout_ttl=None):
    filters = [JobSchedulerInfo.f_job_id == job_id]
    if set_or_reset:
        update_fields = {JobSchedulerInfo.f_ready_signal: True, JobSchedulerInfo.f_ready_time: current_timestamp()}
        filters.append(JobSchedulerInfo.f_ready_signal == False)
    else:
        update_fields = {JobSchedulerInfo.f_ready_signal: False, JobSchedulerInfo.f_ready_time: None}
        filters.append(JobSchedulerInfo.f_ready_signal == True)
        if ready_timeout_ttl:
            filters.append(current_timestamp() - JobSchedulerInfo.f_ready_time > ready_timeout_ttl)
    update_status = JobSchedulerInfo.update(update_fields).where(*filters).execute() > 0
    return update_status


@DB.connection_context()
def cancel_signal(job_id, set_or_reset: bool):
    update_status = JobSchedulerInfo.update({JobSchedulerInfo.f_cancel_signal: set_or_reset, JobSchedulerInfo.f_cancel_time: current_timestamp()}).where(JobSchedulerInfo.f_job_id == job_id).execute() > 0
    return update_status
...
def rerun_signal(job_id, set_or_reset: bool):
    if set_or_reset is True:
        update_fields = {JobSchedulerInfo.f_rerun_signal: True, JobSchedulerInfo.f_cancel_signal: False, JobSchedulerInfo.f_end_scheduling_updates: 0}
    elif set_or_reset is False:
        update_fields = {JobSchedulerInfo.f_rerun_signal: False}
    else:
        raise RuntimeError(f"can not support rereun signal {set_or_reset}")
    update_status = JobSchedulerInfo.update(update_fields).where(JobSchedulerInfo.f_job_id == job_id).execute() > 0
    return update_status
